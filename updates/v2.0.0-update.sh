...
cat << 'RECONCILE_EOF' > /usr/local/bin/omarchy-pi5-post-update
#!/usr/bin/env bash
# ==============================================================================
...
# Deploy pacman hook
cat << 'PACMAN_HOOK_EOF' > /etc/pacman.d/hooks/99-omarchy-pi5.hook
[Trigger]
Operation = Upgrade
Operation = Install
Type = Package
...
[Action]
Description = [Omarchy Quattro] Re-applying Pi 5 hardware safeguards & configurations...
When = PostTransaction
Exec = /usr/local/bin/omarchy-pi5-post-update
PACMAN_HOOK_EOF
